...
def run_from_epg_menu(menuid, **kwargs):
    if menuid == "epg" and config.plugins.epgimport.showinmainmenu.getValue():
        return [(_("Levi45 EPG Import"), main, "epgimporter", 90)]
    else:
        return []
...
def Plugins(**kwargs):
    result = [
        PluginDescriptor(name="Levi45 EPGImport", description=description, where=[PluginDescriptor.WHERE_AUTOSTART, PluginDescriptor.WHERE_SESSIONSTART], fnc=autostart, wakeupfnc=getNextWakeup),
        PluginDescriptor(name=_("Levi45 EPG Import"), description=description, where=[PluginDescriptor.WHERE_PLUGINMENU], icon="plugin.png", fnc=main),
        PluginDescriptor(name="Levi45 EPG importer", description=description, where=[PluginDescriptor.WHERE_MENU], fnc=run_from_epg_menu)
    ]
